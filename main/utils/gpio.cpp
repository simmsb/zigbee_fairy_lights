...
static gpio_mode_t flags_to_mode(gpio::Flags flags) {
  ...
  if (flags == gpio::FLAG_INPUT) {
    ...
    return GPIO_MODE_OUTPUT_OD;
             ...
             (gpio::FLAG_INPUT | gpio::FLAG_OUTPUT | gpio::FLAG_OPEN_DRAIN)) {
...
void ESP32InternalGPIOPin::attach_interrupt(void (*func)(void *), void *arg,
  ...
  if (!isr_service_installed) {
    ...
    if (res != ESP_OK) {
      ESP_LOGE(TAG,
               "attach_interrupt(): call to gpio_install_isr_service() failed, "
               "error code: %d",
               res);
      return;
    }
...
void ESP32InternalGPIOPin::setup() {
  ...
  conf.pull_down_en = flags_ & gpio::FLAG_PULLDOWN ? GPIO_PULLDOWN_ENABLE
...
void ESP32InternalGPIOPin::pin_mode(gpio::Flags flags) {
  // can't call gpio_config here because that logs in esp-idf which may cause
  // issues
