...
#define LOG_FLOAT_OUTPUT(this)                                                 \
  ...
  if (this->max_power_ != 1.0f) {                                              \
    ESP_LOGI(TAG, "  Max Power: %.1f%%", this->max_power_ * 100.0f);           \
  }                                                                            \
  if (this->min_power_ != 0.0f) {                                              \
    ESP_LOGI(TAG, "  Min Power: %.1f%%", this->min_power_ * 100.0f);           \
  }

/** Base class for all output components that can output a variable level, like
 * PWM.
 *
 * Floating Point Outputs always use output values in the range from 0.0 to 1.0
 * (inclusive), where 0.0 means off and 1.0 means fully on. While using floating
 * point numbers might make computation slower, it makes using maths much easier
 * and (in theory) supports all possible bit depths.
 *
 * If you want to create a FloatOutput yourself, you essentially just have to
 * override write_state(float). That method will be called for you with
 * inversion and max-min power and offset to min power already applied.
 *
 * This interface is compatible with BinaryOutput (and will automatically
 * convert the binary states to floating point states for you). Additionally,
 * this class provides a way for users to set a minimum and/or maximum power
 * output
 */
...
public:
   ...
   *
   * All values are multiplied by max_power - min_power and offset to min_power
   ...
   *
   * All values are multiplied by max_power - min_power and offset by min_power
   * to get the adjusted value.
   *
