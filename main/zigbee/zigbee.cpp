...
void ZigBeeComponent::set_report(uint8_t endpoint_id, uint16_t cluster_id,
  ...
  reporting_info.u.send_info.min_interval =
      10; /*!< Actual minimum reporting interval */
  reporting_info.u.send_info.max_interval =
      0; /*!< Actual maximum reporting interval */
  reporting_info.u.send_info.def_min_interval =
      10; /*!< Default minimum reporting interval */
  reporting_info.u.send_info.def_max_interval =
      0; /*!< Default maximum reporting interval */
...
static void bind_cb(esp_zb_zdp_status_t zdo_status, void *user_ctx) {
  ESP_LOGD(TAG,
           "Bind response from address(0x%x), endpoint(%d) with status(%d)",
...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct) {
  ...
  case ESP_ZB_ZDO_SIGNAL_SKIP_STARTUP:
    // Notifies the application that ZBOSS framework (scheduler, buffer pool,
    // etc.) has started, but no join/rejoin/formation/BDB initialization has
    // been done yet.
  ...
  case ESP_ZB_BDB_SIGNAL_DEVICE_FIRST_START:
    ...
    if (err_status == ESP_OK) {
      ...
      ESP_LOGE(TAG,
               "FIRST_START.  Device started up in %sfactory-reset mode with "
               "an error %d (%s)",
               esp_zb_bdb_is_factory_new() ? "" : "non ", err_status,
  ...
  case ESP_ZB_BDB_SIGNAL_STEERING:
    ...
    if (err_status == ESP_OK) {
      ...
      memcpy(&(bind_req.dst_address_u.addr_long), coord.ieee_addr,
  ...
  case ESP_ZB_ZDO_SIGNAL_LEAVE:
    leave_params =
        (esp_zb_zdo_signal_leave_params_t *)esp_zb_app_signal_get_params(
            p_sg_p);
...
void ZigBeeComponent::handle_attribute(esp_zb_device_cb_common_info_t info,
                                       esp_zb_zcl_attribute_t attribute) {
  if (this->attributes_.find({info.dst_endpoint, info.cluster,
                              ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, attribute.id}) !=
      this->attributes_.end()) {
    this->attributes_[{info.dst_endpoint, info.cluster,
                       ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, attribute.id}]
        ->on_value(attribute);
  }
}

void ZigBeeComponent::create_default_cluster(
    uint8_t endpoint_id, esp_zb_ha_standard_devices_t device_id) {
  this->cluster_list_[endpoint_id] =
      esphome_zb_default_clusters_create(device_id);
  this->endpoint_list_[endpoint_id] = device_id;
}

void ZigBeeComponent::add_cluster(uint8_t endpoint_id, uint16_t cluster_id,
                                  uint8_t role) {
...
void ZigBeeComponent::set_basic_cluster(
    std::string model, std::string manufacturer, std::string date,
    uint8_t power, uint8_t app_version, uint8_t stack_version,
    uint8_t hw_version, std::string area, uint8_t physical_env) {
...
esp_zb_attribute_list_t *ZigBeeComponent::create_basic_cluster_() {
  ...
  ESP_LOGI(TAG, "Manufacturer: %s",
           this->basic_cluster_data_.manufacturer.c_str());
  ESP_LOGI(TAG, "Date: %s", this->basic_cluster_data_.date.c_str());
  ESP_LOGI(TAG, "Area: %s", this->basic_cluster_data_.area.c_str());
  uint8_t *ManufacturerName = get_character_string(
      this->basic_cluster_data_
          .manufacturer); // warning: this is in format {length, 'string'} :
  uint8_t *ModelIdentifier =
      get_character_string(this->basic_cluster_data_.model);
  ...
  esp_zb_attribute_list_t *attr_list =
      esp_zb_basic_cluster_create(&basic_cluster_cfg);
  ...
  esp_zb_basic_cluster_add_attr(
      attr_list, ESP_ZB_ZCL_ATTR_BASIC_MANUFACTURER_NAME_ID, ManufacturerName);
  esp_zb_basic_cluster_add_attr(
      attr_list, ESP_ZB_ZCL_ATTR_BASIC_MODEL_IDENTIFIER_ID, ModelIdentifier);
  esp_zb_basic_cluster_add_attr(attr_list, ESP_ZB_ZCL_ATTR_BASIC_DATE_CODE_ID,
                                DateCode);
  esp_zb_basic_cluster_add_attr(
      attr_list, ESP_ZB_ZCL_ATTR_BASIC_LOCATION_DESCRIPTION_ID, Location);
  esp_zb_basic_cluster_add_attr(attr_list,
                                ESP_ZB_ZCL_ATTR_BASIC_PHYSICAL_ENVIRONMENT_ID,
...
esp_err_t
ZigBeeComponent::create_endpoint(uint8_t endpoint_id,
  ...
  return esp_zb_ep_list_add_ep(this->esp_zb_ep_list_, esp_zb_cluster_list,
                               endpoint_config);
}
...
void ZigBeeComponent::esp_zb_task_() {
  ...
  for (auto const &[key, val] : this->attribute_list_) {
    esp_zb_cluster_list_t *esp_zb_cluster_list =
        this->cluster_list_[std::get<0>(key)];
    ret = esphome_zb_cluster_list_add_or_update_cluster(
        std::get<1>(key), esp_zb_cluster_list, val, std::get<2>(key));
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "Could not create cluster 0x%04X with role %u: %s",
               std::get<1>(key), std::get<2>(key), esp_err_to_name(ret));
    }
  }
  ...
  for (auto reporting_info : this->reporting_list) {
    ...
    if (esp_zb_zcl_update_reporting_info(&reporting_info) != ESP_OK) {
      ESP_LOGE(TAG,
               "Could not configure reporting for attribute 0x%04X in cluster "
               "0x%04X in endpoint %u",
               reporting_info.attr_id, reporting_info.cluster_id,
               reporting_info.ep);
    }
    // ESP_ERROR_CHECK(esp_zb_zcl_start_attr_reporting(attr_info));  // is this
    // needed?
  }

  if (esp_zb_set_primary_network_channel_set(ESP_ZB_PRIMARY_CHANNEL_MASK) !=
      ESP_OK) {
  ...
  zb_set_ed_node_descriptor(0, 0, 1);
  this->started_ = true;
...
void ZigBeeComponent::setup() {
  ...
  xTaskCreate(
      [](void *arg) { static_cast<ZigBeeComponent *>(arg)->esp_zb_task_(); },
